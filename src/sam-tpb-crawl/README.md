...
```bash
sam-tpb-crawl$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
sam-tpb-crawl$ sam local start-api
sam-tpb-crawl$ curl http://localhost:3000/
```
...
```bash
sam-tpb-crawl$ sam logs -n HelloWorldFunction --stack-name sam-tpb-crawl --tail
```
...
```bash
sam delete --stack-name sam-tpb-crawl
```
